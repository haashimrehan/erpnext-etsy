...
class MonetaryAmount(BaseModel):
	...
	def __compare(self, other: MonetaryAmount, op: str = "compare") -> tuple[int, int, int]:
		if self.currency_code != other.currency_code:
			raise ValueError(
				f"Cannot {op} monetary amounts with different currencies: {self.currency_code.value} and {other.currency_code.value}"
			)
	...
	def __add__(self, other: MonetaryAmount) -> MonetaryAmount:
		...
		return MonetaryAmount(
			amount=corrected_self + corrected_other, divisor=divisor, currency_code=self.currency_code
		)
	...
	def __mul__(self, other: int | float) -> MonetaryAmount:
		...
		return MonetaryAmount(
			amount=int(self.amount * other), divisor=self.divisor, currency_code=self.currency_code
		)
...
class Transaction(BaseModel):
	...
	paid_timestamp: datetime | None
	shipped_timestamp: datetime | None
	quantity: int
	...
	min_processing_days: int | None
	max_processing_days: int | None
	shipping_method: str | None
	shipping_upgrade: str | None
...
class ShopReceipt(BaseModel):
	...
	message_from_seller: str | None
	message_from_buyer: str | None
	message_from_payment: str | None
	is_paid: bool
	is_shipped: bool
	create_timestamp: datetime
	created_timestamp: datetime
...
class Payment(BaseModel):
	...
	posted_gross: MonetaryAmount | None
	posted_fees: MonetaryAmount | None
	posted_net: MonetaryAmount | None
	adjusted_gross: MonetaryAmount | None
	adjusted_fees: MonetaryAmount | None
	adjusted_net: MonetaryAmount | None
	currency: str
...
class Listing(BaseModel):
	...
	item_weight_unit: WeightUnit | None
	item_length: float | None
	item_width: float | None
	item_height: float | None
	item_dimensions_unit: DimensionsUnit | None
	is_private: bool
	style: list[str]
	file_data: str | None
	has_variations: bool
