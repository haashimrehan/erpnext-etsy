frappe.ui.form.on("Etsy Shop", {
	refresh(frm) {
		...
		if (!frm.is_new()) {
			if (frm.doc.status === "Disconnected") {
				frm.add_custom_button(
					__("Login with {} on Etsy", [frm.doc.shop_name]),
					async () => {
						if (frm.is_dirty()) {
							...
							[
								{
									...
									default: "all",
									description: __(
										"Listings with the selected state will be imported or updated."
									),
								},
								{
									...
									default: 1,
									description: __(
										"Also create or update Item Attributes of the selected Listings."
									),
								},
								{
									...
									default: 0,
									description: __(
										"Also create or update Items & Variants of the selected Listings."
									),
								},
							],
							(values) => {
				...
				frm.add_custom_button(__("Disconnect"), async () => {
					frappe.warn(
						...
						__("You will need to login again with '{}' before next use!", [
							frm.doc.shop_name,
						]),
						() => {
				...
				frappe.db
					.get_single_value("Etsy Settings", "etsy_enabled")
					.then((etsy_enabled) => {
						if (etsy_enabled == 0) {
							frm.set_intro(
								__(
									"Synchronisation is not enabled! 'Etsy Settings > Enable Synchronisation'"
								),
								"yellow"
							);
						}
		...
		frappe.db
			.get_single_value("Selling Settings", "cust_master_name")
			.then((cust_master_name) => {
				if (cust_master_name === "Customer Name") {
					frm.set_intro(
						__(
							"It is not recommended to add new customers by name! 'Selling Settings > Customer Naming By' - using 'Naming Series' is preferred."
						),
						"yellow"
					);
				}
